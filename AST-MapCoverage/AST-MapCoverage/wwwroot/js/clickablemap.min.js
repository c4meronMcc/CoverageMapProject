document.addEventListener('DOMContentLoaded', function () {
        ...
        areas.forEach(area => {
            area.addEventListener('click', function (event) {
                ...
                if (clickedElement.classList.contains('sea-ocean')) {
                    ...
                    clickedElement.style.fill = '#6082B6';
                    clickedElement.classList.add('clicked');
                } else {
                    if (clickedElement.id !== 'backgroundLayer') {
                        const landAreas = mapElement.querySelectorAll('#allLand > .clicked');
                        landAreas.forEach(area => {
                            area.style.fill = '#899499';
                            area.classList.remove('clicked');
                        });
                        clickedElement.style.fill = '#8A9A5B';
                        clickedElement.classList.add('clicked');

                        const landAreas2 = mapElement.querySelectorAll('#allLand > * .clicked');
                        landAreas2.forEach(area => {
                            area.style.fill = '#899499';
                            area.classList.remove('clicked');
                        });
                        clickedElement.style.fill = '#8A9A5B';
    ...
    function handleClick(event) {
        // Reset sea/ocean areas if a land area is clicked
        const seaOceanAreas = mapElement.querySelectorAll('.sea-ocean.clicked');
        seaOceanAreas.forEach(area => {
            area.style.fill = '#B2BEB5';
            area.classList.remove('clicked');
